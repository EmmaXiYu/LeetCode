...
public class LeetCode368 {
	        ...
	        for(int i=1;i<nums.length;i++)
	        {
	            ...
	            List<Integer> lIndex=new ArrayList<>();
	             while(k<i){
	               ...
	               List<Integer> newList=new ArrayList<>(ll.get(k));
	               if(nums[i]%newList.get(newList.size()-1)==0||newList.get(newList.size()-1)%nums[i]==0)
	            		   {
	               newList.add(nums[i]);
	             //  k++;
	                temp.add(newList);
	             if(tempMaxLength<newList.size())
	             {
	                 tempMaxLength=newList.size();
	                 tempMaxIndex=temp.size()-1;
	                 lIndex=new ArrayList<>();
	                 lIndex.add(tempMaxIndex);
	             }
	             else if(tempMaxLength==newList.size())
	             {
	            	 lIndex.add(temp.size()-1);
	             }
	               ifAdd=true;
	           ...
	           k++;
	            ...
	            ll.add(l);}
	            	...
	            	for(int m=0;m<lIndex.size();m++)
	            		
	            ll.add(temp.get(lIndex.get(m)));
	            if(maxLength<temp.get(tempMaxIndex).size())
	            {
	                maxIndex=ll.size()-1;
	            }}
	 ...
	 {
		 LeetCode368 obj=new LeetCode368();
		 ...
		 //int [] nums2={1,2,4,8,16};
		 List<Integer> l=obj.largestDivisibleSubset(nums1);
